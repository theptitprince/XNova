<script language="JavaScript">
    ...
    function fenster(target_url,win_name) {
      if (window.event) window.event.preventDefault(); if (typeof xnova_csrf != 'undefined') target_url += (target_url.indexOf('?') < 0 ? '?' : '&') + 'csrf_token=' + xnova_csrf; window.location.href = target_url; // dans la frame du jeu, avec le jeton CSRF
    }
  </script>
