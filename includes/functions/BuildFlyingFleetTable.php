...
function BuildFlyingFleetTable () {
	...
	while ( $CurrentFleet = mysqli_fetch_assoc( $FlyingFleets ) ) {
		...
		$Bloc['id_label']       = $CurrentFleet['fleet_id'];
		$Bloc['mission_label']  = CreateFleetPopupedMissionLink ( $CurrentFleet, $lang['type_mission'][ $CurrentFleet['fleet_mission'] ], '' );
		$Bloc['mission_label'] .= "<br>". (($CurrentFleet['fleet_mess'] == 1) ? "R" : "A" );

		$Bloc['fleet_label']    = CreateFleetPopupedFleetLink ( $CurrentFleet, $lang['tech'][200], '' );
		...
		$Bloc['st_time']  = date('H:i:s d/m/Y', $CurrentFleet['fleet_start_time']);
		...
		$Bloc['en_time']  = date('H:i:s d/m/Y', $CurrentFleet['fleet_end_time']);
